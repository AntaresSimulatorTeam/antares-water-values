...
def test_create_and_modify_weekly_problem():
    ...
    beta,lamb,_, _, _ = problem.modify_weekly_problem_itr(control=0,i=0,prev_basis=Basis())
    assert beta == pytest.approx(943484691.8759749)
    assert lamb == pytest.approx(-200.08020911704824)

    problem = AntaresProblem(year=0,week=0,path="test_data/one_node",itr=1)
    problem.create_weekly_problem_itr(param=param,reservoir=reservoir,pen_low=0,pen_high=0,pen_final=0)
    beta,lamb,_, _, _ = problem.modify_weekly_problem_itr(control=8400000,i=0,prev_basis=Basis())
