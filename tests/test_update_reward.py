...
def test_init_reward() -> None:
    ...
    assert initial_reward({AreaIndex("0"): 0}) == 10
...
def test_update_reward() -> None:
    ...
    assert initial_reward({AreaIndex("0"): 0}) == 30
    assert initial_reward({AreaIndex("0"): 4}) == 10
    assert initial_reward({AreaIndex("0"): 10}) == 10
